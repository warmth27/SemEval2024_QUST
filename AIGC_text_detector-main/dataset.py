...
class EncodedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        output = self.tokenizer(text, padding='max_length', max_length=self.max_sequence_length, truncation=True,
                                return_tensors='pt')
...
def load_texts_original(data_file):
    ...
    for line in tqdm(data, desc=f'Loading {data_file}'):
        if line['label'] == 0:
            human_texts.append(line['text'])
            # human_qs.append(line['question'])
        else:
            chatgpt_texts.append(line['text'])
            # chatgpt_qs.append(line['question'])
...
def chatgpt_load_datasets(train_data_file, val_data_file, tokenizer, batch_size,
                          ...
                          mode='single', val_file1=None, val_file2=None, val_file3=None, val_file4=None, val_file5=None,
                          val_file6=None, args=None):
    ...
    train_dataset = EncodedDataset(real_train, fake_train, tokenizer, max_sequence_length, min_sequence_length,
                                   epoch_size, token_dropout, seed, args,
                                   train_flag=True)  # in this context, real->label1->chatgpt
    ...
    validation_dataset = EncodedDataset(real_valid, fake_valid, tokenizer, max_sequence_length,
                                        min_sequence_length)  # self added, truncated kept identical to train

    if val_file1 is not None:
        validation_dataset1 = EncodedDataset(real_valid1, fake_valid1, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)
    if val_file2 is not None:
        validation_dataset2 = EncodedDataset(real_valid2, fake_valid2, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)
    if val_file3 is not None:
        validation_dataset3 = EncodedDataset(real_valid3, fake_valid3, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)
    if val_file4 is not None:
        validation_dataset4 = EncodedDataset(real_valid4, fake_valid4, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)
    if val_file5 is not None:
        validation_dataset5 = EncodedDataset(real_valid5, fake_valid5, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)
    if val_file6 is not None:
        validation_dataset6 = EncodedDataset(real_valid6, fake_valid6, tokenizer, max_sequence_length,
                                             min_sequence_length, args=args)

    validation_loader = DataLoader(validation_dataset, batch_size=args.val_batch_size,
                                   sampler=Sampler(validation_dataset))

    if val_file1 is not None:
        validation_loader1 = DataLoader(validation_dataset1, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset1))
    if val_file2 is not None:
        validation_loader2 = DataLoader(validation_dataset2, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset2))
    if val_file3 is not None:
        validation_loader3 = DataLoader(validation_dataset3, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset3))
    if val_file4 is not None:
        validation_loader4 = DataLoader(validation_dataset4, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset4))
    if val_file5 is not None:
        validation_loader5 = DataLoader(validation_dataset5, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset5))
    if val_file6 is not None:
        validation_loader6 = DataLoader(validation_dataset6, batch_size=args.val_batch_size,
                                        sampler=Sampler(validation_dataset6))
